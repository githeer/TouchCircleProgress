...
public class DonutProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        isTouch = attributes.getBoolean(R.styleable.DonutProgress_donut_touch_enable, true);
        touch_ratio = attributes.getFloat(R.styleable.DonutProgress_donut_touch_ratio, 0.5f);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (ty <= radius/* h/2 */) {
                    if (tx - radius > 0) {
                        // 第一象限
                        angle = 360 - getStartingDegree() - d;
                    } else {
                        // 第二象限
                        angle = 180 - getStartingDegree() + d;
                    }
                } else {
                    if (tx - radius > 0) {
                        // 第四象限
                        angle = -getStartingDegree() + d;
                    } else {
                        // 第三象限
                        angle = 180 - getStartingDegree() - d;
                    }
                }
                progress = (int) (angle * 100 / 360);
                if (progress < 0)
                    progress = 0;
                if (progress > 100)
                    progress = 100;
