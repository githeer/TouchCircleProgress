...
public class ArcProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        isTouch = attributes.getBoolean(R.styleable.ArcProgress_arc_touch_enable, true);
        touch_ratio = attributes.getFloat(R.styleable.ArcProgress_arc_touch_ratio, 0.5f);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                float ty = event.getY();
                // 不超过弧形触摸范围
                if (ty > radius * 2 - arcBottomHeight) {
                    break;
                }
                // 不响应内圆一定比例
                float distanceInner = (float) (Math.sqrt(Math.pow(tx - radius, 2) + Math.pow(ty - radius, 2)) / radius);
                if (distanceInner < (1 - touch_ratio)) {
                    break;
                }
                ...
                if (ty <= radius/* h/2 */) {
                    if (tx - radius > 0) {
                        // 第一象限
                        angle = 180 - startAngle + 90 + (90 - d);
                    } else {
                        // 第二象限
                        angle = 180 - startAngle + d;
                    }
                } else {
                    if (tx - radius > 0) {
                        // 第四象限
                        angle = (arcAngle - 180) / 2 + 180 + d;
                    } else {
                        // 第三象限
                        angle = (arcAngle - 180) / 2 - d;
                    }
                }
                progress = (int) (angle / arcAngle * getMax());
                if (progress < 0)
                    progress = 0;
                if (progress > 100)
                    progress = 100;
